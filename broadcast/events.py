...
from .app import channels, socketio
from .schemas import (
...
def handle_join_viewer(data=None):
    ...
    if not channel_id:
        ...
        active_channels = [
            ...
            for cid, cdata in list(
                channels.items()
            )  # list() to avoid dict size change during iteration
        ]
    ...
    print(
        f"Viewer joined channel {channel_id}: {request.sid} (Total: {len(channel_data['viewers'])})"
    )
    emit("broadcaster_available", {"broadcaster_id": broadcaster_id, "channel_id": channel_id})
...
def handle_offer(data):
    ...
    emit("offer", {"sdp": data["sdp"], "sender": request.sid}, room=target)
...
def handle_answer(data):
    ...
    emit("answer", {"sdp": data["sdp"], "sender": request.sid}, room=target)
...
def handle_ice_candidate(data):
    ...
    target = data.get("target")
    if target:
        emit("ice_candidate", {"candidate": data["candidate"], "sender": request.sid}, room=target)
...
def handle_send_message(data):
    ...
    viewer_name = channels[channel_id]["viewer_aliases"].get(
        request.sid, f"Viewer-{request.sid[:8]}"
    )
    ...
    emit(
        "new_message",
        {"sender_id": viewer_name, "message": message, "timestamp": time.time()},
